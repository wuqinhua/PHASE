...
sample_list_df = pd.read_csv(r"./sampleid_560.csv")
sample_list = sample_list_df['sample_id'].values.tolist()
print(sample_list)
...
allData = COVID_Dataset(DataList, DataLabel)
print(DataLabel)
...
for i, data in enumerate(allLoader, 0):
    inputs, labels = data
    labels = labels.long()
    print("input",inputs[0].shape,labels.shape)
...
feature_name = gene_list

df_fea = pd.DataFrame(ig_attr_list)
df_fea = df_fea.T
index_dict = dict(zip(df_fea.index, gene_list))
column_dict = dict(zip(df_fea.columns,sample_list))
df_fea.rename(index=index_dict,columns=column_dict,inplace=True)
print(df_fea)
df_fea = df_fea.T
df_fea.to_csv(r"./Analysis_result/Attr_result/attr_sample_PHASE.csv")
print('IG Finished!')
